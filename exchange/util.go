...
// 	if known.ClientUUID == "" && update.ClientUUID != "" {
// 		tmp.ClientUUID = update.ClientUUID
// 	}
...
func (v *SimpleOrder) String() string {
	return fmt.Sprintf("{OrderID: %s ClientID %s Side %s Price %s Size %s Fee %s Status %s CreatedAt %s}",
		v.ServerOrderID, v.ClientUUID, v.Side, v.FilledPrice.StringFixed(3), v.FilledSize.StringFixed(3), v.Fee.StringFixed(3), v.Status, v.CreateTime.Time.Format(time.DateTime))
}
